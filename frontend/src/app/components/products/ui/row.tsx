...
function Row({
  ...
  const handleToggle = () => {
    setSelectedProducts((prev) => {
      const alreadySelected = prev.some(
        (product) => product?.product_id === selectedData?.product_id
      );
      if (alreadySelected) {
        return prev.filter(
          (product) => product?.product_id !== selectedData?.product_id
  ...
  async function HandleDeleteProducts(e: FormEvent) {
    ...
    const response = await DeleteProductAction(data?.product_id);
      ...
      <Dialog.Root open={isViewImage} onOpenChange={setIsViewImage}>
        ...
        <Dialog.Content>
          <div className="relsative">
            ...
            <Image
              src={BASE_URL + "/" + data?.product_cover}
              alt="img"
              height={500}
              width={600}
            />
    ...
    <tr>
      ...
      >
        <div className="flex items-center gap-2 relative">
          ...
          <span className="text-base ml-6">{data?.product_id}</span>
        </div>
      </td>
      <td
        style={{ padding: cellPadding }}
        className="border border-neutral-200"
      >
        <span className="text-xs">{data?.product_title}</span>
      </td>
      <td
        style={{ padding: cellPadding }}
        className="border border-neutral-200"
      >
        <span className="text-xs">{data?.product_description}</span>
      </td>
      <td
        style={{ padding: cellPadding }}
        className="border   border-neutral-200"
